...
코퍼스에는 같은 보고서가 **두 판** 들어 있습니다. 하나는 이미 대체된 구판이고, 하나는
수정본입니다. 둘은 줄 번호까지 같고 수치 몇 개만 다릅니다. 파일이 인덱스에 두 번 들어간
것뿐이고, **문서는 자기가 대체된 줄 모릅니다.** 실무의 코퍼스가 꼭 이렇습니다.

그래서 검색은 두 판을 다 물어오고, 모델은 어느 쪽이 살아 있는지 판단할 근거가 없습니다.
...
질문 세트(`golden.py`)는 **답이 어느 판을 봤느냐에 따라 갈리는 질문**으로 앞의 셋을 채우고,
뒤의 셋은 두 판에서 값이 같은 대조군으로 두었습니다.
...
읽지 않는다는 것이 "step 1 에는 판 개념이 없다" 의 코드상 의미입니다.
...
| 결정론 · 판 | 대체된 문서에만 있는 수치를 말하고 있나 | 모델 호출 0번 |
...
판 검사가 묻는 것이 **"폐기된 문서를 봤나" 가 아니라 "그 수치가 살아 있는 판에도 있나"**
라는 점을 눈여겨보세요. 앞으로 물으면 두 판에서 값이 똑같은 수치까지 되돌리게 되고,
...
harness/
  ...
  harness_verify.py    검증 레이어. 결정론 검사(인용 · 판) + 근거 판정
  harness_metrics.py   공용 계측. ToolResult · 지표 계산 · 리포트 형식
  golden.py            질문 6개 · 근거 · 판 목록. 근거는 실행 시점에 현행 판에서 잘라낸다
...
assets/tools/harness/  코퍼스. 같은 보고서의 두 판
  KB-2510-4471.md      대체된 구판
  KB-2510-8092.md      현행 수정본 — 수치 3개가 다르고, 줄 번호는 같다
```

파일 이름에 판 정보가 없는 것은 의도입니다. `_수정본` 이나 `_v2` 로 두면 모델이 이름만 보고
최신을 골라 버려서, 이 랩이 보여주려는 실패가 일어나지 않습니다. 문서 **본문에도** 판 표시가
없습니다.
