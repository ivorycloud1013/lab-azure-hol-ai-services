...
질문을 고른 기준이 이 랩의 전부다. **답이 어느 판을 봤느냐에 따라 갈리는 질문**만 골랐다.

코퍼스에는 같은 보고서가 두 판 들어 있다. 하나는 이미 대체된 구판이고, 하나는 수정본이다.
...
step 2 부터만 읽는다. step 1 이 이 표를 읽지 않는다는 것이 "step 1 에는 판 개념이 없다" 의
코드상 의미다.
...
# source_lines 는 답이 있는 자리이고, 그 줄의 본문은 실행 시점에 현행 판에서 잘라온다.
GOLDEN = [
    {
        # 구판 409.2 → 수정본 386.5. 이 수치는 본문에 없고 그림 18 안에만 있어서, 검색하면
        # 두 판의 그림이 나란히 돌아온다. 값이 다르다는 것은 보이지만 어느 쪽이 유효한지는
        # 어디에도 없다.
    ...
    },
    {
        ...
        # 검색 한 번에 네 줄(두 판 × 두 자리)이 돌아온다. 가장 헷갈리는 모양이다.
    ...
    {
        # 구판 152.1 → 수정본 149.7. 한 줄에만 있어서 두 판의 차이가 가장 또렷하게 보인다.
    ...
    {
        # 여기부터 대조군. 두 판에서 값이 같아 하네스가 잡을 것이 없다. 대조군이 있어야
...
def is_superseded(document):
    """이 문서가 대체된 판인가. 하네스만 답할 수 있는 질문이다."""
    return document in SUPERSEDED_EDITIONS


def resolve_golden(corpus, count=None):
    """근거 문단을 여기 적어두지 않고 현행 판에서 잘라온다.

    베껴 두면 읽기는 좋고 조용히 틀린다. 문서를 다시 추출하는 순간 사본이 어긋나고, 그때부터
    채점 기준이 코퍼스에 있지도 않은 문장이 된다. 모든 단계가 계속 숫자를 찍고, 그 숫자가
    전부 거짓말이 된다.
    """
    lines = corpus[CURRENT_EDITION]["lines"]
    resolved = []
...
def is_hit(item, text):
    ...
    사라진다. 어느 판이 유효한지 고르는 것이 여기서 요구하는 일이다.
...
def citations(text):
    ...
    step 0 에서 지어낸 인용을 세고, step 2 에서 폐기된 판을 인용했는지 보는 데 쓴다.
    """
